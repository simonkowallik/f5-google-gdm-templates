...
#
# Version v1.5.0_simonkowallik_git.io/fAZZj
...
def GenerateConfig(context):
  ...
  if ALLOWUSAGEANALYTICS == "yes":
      ...
      SENDANALYTICS = ' --metrics "cloudName:google,region:' + context.properties['region'] + ',bigipVersion:' + context.properties['imageName'] + ',customerId:${CUSTOMERID},deploymentId:${DEPLOYMENTID},templateName:f5-existing-stack-byol-3nic-bigip.py,templateVersion:v1.5.0_simonkowallik_git.io/fAZZj,licenseType:byol"'
  else:
      CUSTHASH = ''
      SENDANALYTICS = ''
  resources = [{
          ...
          'metadata': {
              'items': [{
                  ...
                  'value': (''.join(['#!/bin/bash\n',
                                    ...
                                    ' if [ ! -f /config/startupFinished.stage2 ]; then\n',
                                    '   nohup /config/cloud/gce/network-setup.sh > /var/log/cloud/google/network-setup.log 2>&1 &\n',
                                    '   touch /config/startupFinished.stage2\n',
                                    ' fi\n',
                                    ' exit\n',
                                    'fi\n',
                                    'mkdir -p /config/cloud/gce/cfg_state\n',
                                    ...
                                    '\"$@\"\n',
                                    'EOF\n',
                                    'cat <<\'EOF\' > /config/cloud/gce/network-setup.sh\n',
                                    '#!/bin/bash\n',
                                    '# wait for mcpd to come up\n',
                                    'checks=0\n',
                                    'while [ $checks -lt 120 ]; do echo checking mcpd\n',
                                    '    tmsh -a show sys mcp-state field-fmt | grep -q running\n',
                                    '    if [ $? == 0 ]; then\n',
                                    ...
                                    '    sleep 10\n',
                                    'done\n',
                                    '# load nic0 configuration state\n',
                                    'export nic0_ipaddr=$(</config/cloud/gce/cfg_state/nic0_ipaddr)\n',
                                    'export nic0_gateway=$(</config/cloud/gce/cfg_state/nic0_gateway)\n',
                                    'export nic0_netmask=$(</config/cloud/gce/cfg_state/nic0_netmask)\n',
                                    '# load nic1 configuration state\n',
                                    'export nic1_ipaddr=$(</config/cloud/gce/cfg_state/nic1_ipaddr)\n',
                                    'export nic1_gateway=$(</config/cloud/gce/cfg_state/nic1_gateway)\n',
                                    'export nic1_netmask=$(</config/cloud/gce/cfg_state/nic1_netmask)\n',
                                    '# load nic2 configuration state\n',
                                    'export nic2_ipaddr=$(</config/cloud/gce/cfg_state/nic2_ipaddr)\n',
                                    'export nic2_gateway=$(</config/cloud/gce/cfg_state/nic2_gateway)\n',
                                    'export nic2_netmask=$(</config/cloud/gce/cfg_state/nic2_netmask)\n',
                                    '# calculate network addresses\n',
                                    'export nic0_network=$(/bin/ipcalc -n ${nic0_ipaddr} ${nic0_netmask} | cut -d= -f2)\n',
                                    'export nic1_network=$(/bin/ipcalc -n ${nic1_ipaddr} ${nic1_netmask} | cut -d= -f2)\n',
                                    'export nic2_network=$(/bin/ipcalc -n ${nic2_ipaddr} ${nic2_netmask} | cut -d= -f2)\n',
                                    'function error_exit {\n',
                                    ...
                                    'date\n',
                                    'declare -a tmsh=()\n',
                                    'echo \'starting network-setup.sh\'\n',
                                    'useServiceDiscovery=\'',
                                    ...
                                    '\"tmsh create sys management-ip ${nic2_ipaddr}/32\"\n',
                                    '\"tmsh create sys management-route mgmt_gw network ${nic2_gateway}/32 type interface\"\n',
                                    '\"tmsh create sys management-route mgmt_net network ${nic2_network}/${nic2_netmask} gateway ${nic2_gateway}\"\n',
                                    '\"tmsh create sys management-route default gateway ${nic2_gateway}\"\n',
                                    '\"tmsh create net vlan external interfaces add { 1.0 } mtu 1460\"\n',
                                    '\"tmsh create net self self_external address ${nic0_ipaddr}/32 vlan external\"\n',
                                    '\"tmsh create net route ext_gw_int network ${nic0_gateway}/32 interface external\"\n',
                                    '\"tmsh create net route ext_rt network ${nic0_network}/${nic0_netmask} gw ${nic0_gateway}\"\n',
                                    '\"tmsh create net route default gw ${nic0_gateway}\"\n',
                                    '\"tmsh create net vlan internal interfaces add { 1.1 } mtu 1460\"\n',
                                    '\"tmsh create net self self_internal address ${nic1_ipaddr}/32 vlan internal\"\n',
                                    '\"tmsh create net route int_gw_int network ${nic1_gateway}/32 interface internal\"\n',
                                    '\"tmsh create net route int_rt network ${nic1_network}/${nic1_netmask} gw ${nic1_gateway}\"\n',
                                    ...
                                    'date\n',
                                    'EOF\n',
                                    'cat <<\'EOF\' > /config/cloud/gce/save-cfgstate.sh\n',
                                    '#!/bin/bash\n',
                                    'date\n',
                                    'echo \'starting save-cfgstate.sh\'\n',
                                    '# get interface info for nic0 (external)\n',
                                    'echo -n `curl -s -f --retry 20 \"http://metadata.google.internal/computeMetadata/v1/instance/network-interfaces/0/ip\" -H \"Metadata-Flavor: Google\"` > /config/cloud/gce/cfg_state/nic0_ipaddr\n',
                                    'echo -n `curl -s -f --retry 20 \"http://metadata.google.internal/computeMetadata/v1/instance/network-interfaces/0/gateway\" -H \"Metadata-Flavor: Google\"` > /config/cloud/gce/cfg_state/nic0_gateway\n',
                                    'echo -n `curl -s -f --retry 20 \"http://metadata.google.internal/computeMetadata/v1/instance/network-interfaces/0/subnetmask\" -H \"Metadata-Flavor: Google\"` > /config/cloud/gce/cfg_state/nic0_netmask\n',
                                    '# get interface info for nic1 (internal)\n',
                                    'echo -n `curl -s -f --retry 20 \"http://metadata.google.internal/computeMetadata/v1/instance/network-interfaces/1/ip\" -H \"Metadata-Flavor: Google\"` > /config/cloud/gce/cfg_state/nic1_ipaddr\n',
                                    'echo -n `curl -s -f --retry 20 \"http://metadata.google.internal/computeMetadata/v1/instance/network-interfaces/1/gateway\" -H \"Metadata-Flavor: Google\"` > /config/cloud/gce/cfg_state/nic1_gateway\n',
                                    'echo -n `curl -s -f --retry 20 \"http://metadata.google.internal/computeMetadata/v1/instance/network-interfaces/1/subnetmask\" -H \"Metadata-Flavor: Google\"` > /config/cloud/gce/cfg_state/nic1_netmask\n',
                                    '# get interface info for nic2 (internal)\n',
                                    'echo -n `curl -s -f --retry 20 \"http://metadata.google.internal/computeMetadata/v1/instance/network-interfaces/2/ip\" -H \"Metadata-Flavor: Google\"` > /config/cloud/gce/cfg_state/nic2_ipaddr\n',
                                    'echo -n `curl -s -f --retry 20 \"http://metadata.google.internal/computeMetadata/v1/instance/network-interfaces/2/gateway\" -H \"Metadata-Flavor: Google\"` > /config/cloud/gce/cfg_state/nic2_gateway\n',
                                    'echo -n `curl -s -f --retry 20 \"http://metadata.google.internal/computeMetadata/v1/instance/network-interfaces/2/subnetmask\" -H \"Metadata-Flavor: Google\"` > /config/cloud/gce/cfg_state/nic2_netmask\n',
                                    'date\n',
                                    'EOF\n',
                                    'cat <<\'EOF\' > /config/cloud/gce/change-mgmteth.sh\n',
                                    '#!/bin/bash\n',
                                    'date\n',
                                    'echo \'starting change-mgmteth.sh\'\n',
                                    'declare -a tmsh=()\n',
                                    'tmsh+=(',
                                    ...
                                    '    fi\n',
                                    'done\n',
                                    '# change managementeth for ASM\n',
                                    'sed -i -e "s/eth0/eth2/g" /etc/ts/common/image.cfg\n',
                                    'date\n',
                                    '# fix for ID554506, see K47835034 and 1-3821358926/#1-1V1AQLV\n',
                                    'echo \'alert mgmt-mtu-change-ID554506 \"Tmm ready\" { exec command=\"/sbin/ip link set eth2 mtu 1460\" }\' >> /config/user_alert.conf\n',
                                    '# remove interface 1.2\n',
                                    'perl -pi -0777e \'s/^net interface 1.2\\s.+?^\\}$//gms\' /config/bigip_base.conf\n',
                                    '# reboot to activate management interface change\n',
                                    'reboot -f\n',
                                    'EOF\n',
                                    ...
                                    'chmod 755 /config/cloud/gce/save-cfgstate.sh\n',
                                    'chmod 755 /config/cloud/gce/change-mgmteth.sh\n',
                                    'chmod 755 /config/cloud/gce/network-setup.sh\n',
                                    ...
                                    'nohup /config/waitThenRun.sh f5-rest-node /config/cloud/gce/node_modules/f5-cloud-libs/scripts/runScript.js --file /config/cloud/gce/rm-password.sh --cwd /config/cloud/gce -o /var/log/cloud/google/rm-password.log --log-level debug --wait-for ONBOARD_DONE --signal PASSWORD_REMOVED &>> /var/log/cloud/google/cloudlibs-install.log < /dev/null &\n',
                                    'nohup /config/waitThenRun.sh f5-rest-node /config/cloud/gce/node_modules/f5-cloud-libs/scripts/runScript.js --file /config/cloud/gce/save-cfgstate.sh --cwd /config/cloud/gce -o /var/log/cloud/google/save-cfgstate.log --log-level debug --wait-for PASSWORD_REMOVED --signal SAVE_CFGSTATE_DONE &>> /var/log/cloud/google/cloudlibs-install.log < /dev/null &\n',
                                    'nohup /config/waitThenRun.sh f5-rest-node /config/cloud/gce/node_modules/f5-cloud-libs/scripts/runScript.js --file /config/cloud/gce/change-mgmteth.sh --cwd /config/cloud/gce -o /var/log/cloud/google/change-mgmteth.log --log-level debug --wait-for SAVE_CFGSTATE_DONE --signal CHANGE_MGMTETH_DONE &>> /var/log/cloud/google/cloudlibs-install.log < /dev/null &\n',
